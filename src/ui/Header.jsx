...
export const Header = () => {
  ...
  const headerCards = [
    {
      ...
      icon: (
        <Player
          className="h-[70pt] w-[70pt] p-0"
          src="./assets/lotties/Leaf3.json"
          loop
          autoplay
        />
      ),
...

    {
      ...
      icon: (
        <Player
          className="h-[70pt] w-[70pt] p-0"
          src="./assets/lotties/Sunlight.json"
          loop
          autoplay
        />
      ),
  ...
  return (
    <div className="relative">
      <div
        className="w-full min-h-screen bg-black/80 text-orange-50 flex flex-col-reverse lg:flex-row justify-around items-center bg-center bg-cover bg-blend-overlay py-[100pt]"
        ...
        <div>
          <div className="flex flex-row lg:flex-col space-x-4 lg:space-y-4 lg:space-x-0">
            {slides.map((slide) => (
              <button
                key={slide.id}
                onClick={() => switchTab(slide.id)}
                className={`${
                  activeSlide == slide.id
                    ? "bg-gray-50"
                    : "bg-gray-200/30 border-gray-200/50"
                } hover:bg-gray-50 cursor-pointer transition w-[14pt] h-[5pt] lg:w-[5pt] lg:h-[14pt] rounded-full border-2 `}
              ></button>
            ))}
          </div>
        </div>
        ...
        <div className="w-full lg:w-[80%] px-5 pt-10 mt-10 lg:mt-0 lg:pt-0">
          <div className="w-full md:w-2/3 lg:w-2/3 mb-6">
            <h2 className="text-[40pt] pb-5 md:text-[60pt] lg:text-[80pt] font-extrabold leading-tight">
              ...
              <img
                loading="lazy"
                className="hidden lg:inline ml-5 w-[150pt] h-[65pt] rounded-full grayscale hover:grayscale-0 cursor-pointer transition duration-300 hover:animate-pulse"
          ...
          <div className="w-full flex items-center space-x-3">
            <a
              href={slides[activeSlide].cta.link}
              className="text-black bg-white px-8 py-3 lg:py-5 lg:px-10 rounded-2xl text-sm lg:text-md font-bold hover:bg-transparent hoer:border-white border-2 hover:text-white transition duration-300"
            >
