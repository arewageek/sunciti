...
import { FaLightbulb, FaRegLightbulb, FaSolarPanel } from 'react-icons/fa'
...
export const MoreOnMainone = () => {
    ...
    const services = [
        ...
        {
            title: 'Electrical Design ad Wiring',
            content: '',
            icon: <FaRegLightbulb />
        }
    ]
    ...
    return (
        <div className='w-full py-5 my-4'>
            ...
            <div className='md:w-[70%] mx-auto my-5 flex flex-wrap overflow-auto px-5 py-3'>
                {   services.map(service => <MoreOnMainoneCard key={service.title} title={service.title} content={service.content} icon={service.icon} />)  }
            </div>
        </div>
    )
}
